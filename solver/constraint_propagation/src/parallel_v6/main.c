...
struct node ****extended_grids;	/* Per-threadgroup shared extended grid, indexed by tg_id */
...
void *pthreads_solver(void *arg)
{
	...
	if (thread_id == 1) {
		...
		for (i = ta->start; i <= ta->end; ++i) {
			...
			parallel_sudoku_solver(extended_grids[tg_id],
			...
			for (r = 0; r < sudoku_size; r++) {
				for (c = 0; c < sudoku_size; c++) {
					struct node *temp =
						extended_grids[tg_id][r][c];
					if (temp != NULL && temp->next == NULL)
						common_grids[tg_id][r][c] =
							temp->data;
				}
			}
...
int main(int argc, char **argv)
{
	...
	* ******************************************************************/
	...
	if (local_threads > 0 && local_num_lines > 0) {
		...
		extended_grids = (struct node ****)
			malloc(n_threadgroups * sizeof(struct node ***));
